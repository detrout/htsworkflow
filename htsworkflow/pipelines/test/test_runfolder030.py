...
class RunfolderTests(TestCase):
    ...
    def test_firecrest(self):
        ...
        self.assertEqual(f.date,     f2.date)
    ...
    def test_gerald(self):
        ...
        self.assertEqual(g.version, '1.68.2.2')
        self.assertEqual(g.date, datetime(2008,4,19,19,8,30))
        ...
        for i in range(1,9):
            cur_lane = g.lanes[i]
            self.assertEqual(cur_lane.analysis, 'eland')
            self.assertEqual(cur_lane.eland_genome, genomes[i])
            self.assertEqual(cur_lane.read_length, '32')
            self.assertEqual(cur_lane.use_bases, 'Y'*32)
